...
static int litex_uart_console_setup(struct console *co, char *options)
{
	...
	if (!port->mapbase) {
		pr_info("console on %s%i not present\n", LITEX_UART_NAME,
				co->index);
		return -ENODEV;
	}
...
static void early_litex_uart_putc(struct uart_port *port, int c)
{
	...
	while (readl(port->membase + 0x4)) {
		spin_unlock_irqrestore(&port->lock, flags);
		cpu_relax();
		spin_lock_irqsave(&port->lock, flags);
	}
	writeb(c & 0xff, port->membase + 0x00);
...
OF_EARLYCON_DECLARE(litex_uart_a, "litex,litex-uart", early_litex_uart_setup);
